...
* **🎨 图层编辑模式 (v2.0 新增)**: Photoshop 风格的多图层管理系统
    * **🤖 K-Means 自动分层**: 自动识别图中所有颜色曲线，一键生成多个图层
    * **🪄 SAM 智能分割**: 集成 Segment Anything Model，点击即可智能识别区域
    * **🖌️ 画笔/橡皮工具**: 手动修补断裂曲线或擦除噪点
    * **📊 动量追踪算法**: 解决曲线交叉点和高频震荡区域提取问题
    * **🔄 批量提取**: 一次分层，多条曲线批量提取
...
1. **上传图片并校准**（同传统模式）
2. **切换到图层编辑模式**：点击右上角"🎨 图层编辑模式"按钮
3. **自动分层**：点击"🎨 自动分层"按钮，系统自动识别所有颜色曲线
4. **编辑图层**：
   - 选择图层（点击图层列表）
   - 使用工具编辑：
     - 🖌️ **画笔**：修补断裂的曲线
     - 🧼 **橡皮**：擦除噪点或混入的其他颜色
     - 🪄 **魔棒**：点击图片，AI 智能识别并合并到当前图层
5. **提取数据**：点击"📊 提取当前图层数据"
6. **重复步骤 4-5** 处理其他图层
7. **导出 Excel**
...
| 功能 | 传统模式 | 图层编辑模式 |
|------|----------|-------------|
| 多条曲线 | 需重复操作 | 一次分层，批量提取 |
| 重叠曲线 | 困难 | 自动分离到不同图层 |
| 断点修复 | 手动连接 | 画笔 + 动量追踪 |
| 交叉点 | 容易出错 | 动量追踪算法自动处理 |
| 效率 | 10-30秒/条 | 5-15秒/条 |

**详细教程：** 查看 [LAYER_EDITOR_GUIDE.md](./LAYER_EDITOR_GUIDE.md)
